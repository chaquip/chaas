...
// Define secrets
const chaquipApiKey = defineSecret('CHAQUIP_API_KEY');
const sumupApiKey = defineSecret('SUMUP_API_KEY');
const sumupMerchantCode = defineSecret('SUMUP_MERCHANT_CODE');

export const getPaymentLinkForSlackUser = onRequest(
  {
    secrets: [chaquipApiKey, sumupApiKey, sumupMerchantCode],
  },
  async (req, res) => {
    ...
    const apiKey = chaquipApiKey.value();
    ...
    try {
      ...
      if (accountsSnapshot.empty) {
        res
          .status(404)
          .json({error: 'No account found with that Slack user ID'});
        return;
      }
